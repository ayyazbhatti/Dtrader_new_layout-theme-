  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        {stats.map(stat => {
          ...
          return (
            <div
              key={stat.name}
              className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-6 overflow-hidden"
            >
              ...
              <div className="flex items-center mt-3 sm:mt-4">
                ...
                <span className="text-sm text-gray-500 dark:text-gray-400 ml-1 hidden sm:block">
                  from last week
                </span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        {/* Chart Section */}
        <div className="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 space-y-2 sm:space-y-0">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
              Portfolio Performance
            </h2>
            <div className="flex space-x-1 sm:space-x-2">
              <button className="px-3 py-1 text-sm bg-primary-100 dark:bg-primary-900 text-primary-700 dark:text-primary-300 rounded-md">
                1D
              </button>
            ...
            {recentTrades.map((trade, index) => (
              ...
              >
                ...
                <div className="text-right flex-shrink-0 ml-2">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">{trade.time}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {[
              ...
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 truncate">{holding.name}</p>
                </div>
                <div className="text-right flex-shrink-0 ml-2">
                  <p className="font-medium text-gray-900 dark:text-white text-sm sm:text-base">{holding.value}</p>
                  <p className="text-sm text-success-600 dark:text-success-400">{holding.change}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={index} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <div className="flex items-start justify-between">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{news.time}</p>
                  </div>
