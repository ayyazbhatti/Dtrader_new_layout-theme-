      ...
      <div className="px-3 sm:px-4 lg:px-6 py-3 sm:py-4 lg:py-6 space-y-3 sm:space-y-4 lg:space-y-6">
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2 sm:gap-3 lg:gap-4">
          {pageConfig.stats.map((stat, index) => {
            ...
            return (
              <div key={index} className="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-2 sm:p-3 lg:p-4 xl:p-6 hover:shadow-md transition-shadow duration-200">
                <div className="flex items-center justify-between">
                  <div className="min-w-0 flex-1 mr-2 sm:mr-3">
                    ...
                    <p className="text-sm sm:text-base lg:text-lg xl:text-xl font-bold text-gray-900 dark:text-white mt-1 truncate">{stat.value}</p>
                  </div>
                  <div className={`p-1 sm:p-1.5 sm:p-2 lg:p-3 rounded-lg flex-shrink-0 ${getColorClasses(stat.color)}`}>
                    <IconComponent className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 xl:w-6 xl:h-6" />
                  </div>
                </div>
